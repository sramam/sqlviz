...
module.exports = function() {
    ...
    var get_schema = function(db_name, output, host, user, password) {
        ...
        db.query('select table_name, column_name, data_type from information_schema.columns where table_schema = database() order by table_name,ordinal_position;')
            .then(function(response) {
                ...
                var tpl_dir = path.join(__dirname, '../tpl');
                nunjucks.configure(tpl_dir, { autoescape: true});
                var dot_str = nunjucks.render(path.join(tpl_dir, 'sqlviz.tpl'), graph);
                ...
                exec('which dot', function(err, stdout, stderr) {
                    if (!err && !stderr) {
                        ...
                        var fs = require('fs'),
                            ...
                            out_filename = output || (db_name + 'viz.png'),
                            dot = stdout.trim(),
